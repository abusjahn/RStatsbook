...
pacman::p_load(
  ...
  car, broom, multcomp, DescTools, haven,
  ggbeeswarm,
  lme4, nlme, merTools,
  easystats, patchwork
)
...
```{r}
lme_out <- nlme::lme(Growth ~ Treatment,
  data = rawdata,
  random = ~ 1 | Passage_F
) # RIntercept
lme_out
...
p1 <- rawdata |>
  ...
  mutate(Treatment_n = recode_values(Treatment, "Control" ~ 0,
    default = 1
  )) |>
...
With the penguin data, build random intercept and random slope models with bill depth as dependent and weight as independent variable. Treat species (and possibly sex) as the random effects, in which intercept / slope are nested.
