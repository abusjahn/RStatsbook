...
Naming things is harder than you may expect. Try to be verbose and consistent in language and style. Commonly used are snake_case_style, CamelCaseStyle, and kebab-case-style.
...
mtcars |> # inbuild example data, use F1!
  ...
  pivot_longer(cols = -am, 
               names_to = "Measure") |> # put variables in rows
  pivot_wider(
    id_cols = Measure, 
    names_from = am, # put groups in cols
    values_from = value
  )
```
...
# source("somepath/R/myfunctions.R")
```
...
print(my1.Matrix)
data <- seq(from = 1, to = 200, by = 1) # 1:100
nrow <- 20
matrix(
  data = data,
  nrow = nrow,
...
rawdata |>
  ...
  slice(1:5)
rawdata |> select(contains("RR", ignore.case = FALSE))
...
rawdata |> slice(1:5)
rawdata |> slice_tail()
rawdata |> slice_max(order_by = `sysRR (mmHg)`, n = 5)
rawdata |> slice_sample(n = 5)
rawdata |> filter(Sex == "male", `sysRR (mmHg)` > 140) #drops both NA and FALSE
rawdata |> filter_out(Sex == "male", `sysRR (mmHg)` > 140) #keeps both NA and FALSE
```
...
#### case_when / replace_when / recode_values / replace_values

When there are many tests to do, `case_when`/`case_replace` or `recode_values`/`replace_values` (`case_match` is deprecated) are nice replacements for `ifelse.` While case_when allows complex conditions, recode_values is used for simple comparisons:
...
rawdata <- mutate(
  ...
  # `prescribe something?` = recode_values( #case_match(
  #   Hypertension,
  #   "hypertensive" ~ "yes",  # lookup value ~ new value
  #   "borderline" ~ "possibly",
  #   "normotensive" ~ "no"
  # )
  `prescribe something?` = recode_values(
    Hypertension,
    from = c("hypertensive", "borderline", "normotensive"), # lookup-tables
    to = c("yes", "possibly", "no")
  )
)
rawdata |>
