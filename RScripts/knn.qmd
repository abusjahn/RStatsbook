...
## Scaling
To avoid the effect of different scales of the variables, it is desirable to scale the variables. Depending on the distribution of the independent / predicor variables, there are a number of approaches to make canter and spread/range of the data comparable.
Here we first use the `preProcess` function from the `caret` package. 
To compare the scaled data to the original, we'll keep the original data in the data frame.
...
Now we'll try to predict the sex based on body measures and species.
