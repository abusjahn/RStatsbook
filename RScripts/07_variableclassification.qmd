...
ggplot(rawdata, aes(`Size (cm)`)) +
  ...
  scale_x_continuous(
    limits = c(
      mean(rawdata$`Size (cm)`) - 3 * sd(rawdata$`Size (cm)`),
      mean(rawdata$`Size (cm)`) + 3 * sd(rawdata$`Size (cm)`)
    )
  )
...
Side note on anonymous functions:

`mean_function <- function(x) mean(x, na.rm = TRUE)`\
would create a function `mean_function` with argument `x`.

`\()` is just a shorthand for the same thing\
`\(x) mean(x, na.rm = TRUE)`\
creates an unnamed / anonymous function. Anonymous functions let you define a quick, one-off transformation inline — without naming it. They're most useful inside across(), where you want to apply something to multiple columns at once.\
`across()` iterates over selected columns and applies a function to each. When you just need mean or sum, you may pass the function directly. But when you need extra arguments, an anonymous function wraps it.\
The `\(x)` form is now preferred over \~mean(.x,...) because it's plain R, works without loading purrr, and makes the argument explicit — which avoids confusion when functions have multiple arguments.
...
for (var_i in numvars$names) {
  ...
  resulttable2 <- add_row(resulttable2,
    ...
    pKS_Placebo = ks_tmp[[1]] |>
      formatP(mark = TRUE), # added rounding/formatting
    pKS_Verum = ks_tmp[[2]] |>
      formatP(mark = TRUE)
  )
}
...
resulttable1a <-
  ...
  summarize(across(
    .cols = # where(is.numeric),
      all_of(numvars$names[-(1:4)]),
    .fns = list(
      pKS = \(x) ksnormal(x) |>
        formatP(mark = TRUE),
      pSh = \(x) shapiro.test(x) |>
        pluck("p.value") |>
        formatP(mark = TRUE)
    )
  )) |>
