...
pacman::p_load(
  ...
  car, nlme, broom, flextable,
  multcomp,  foreign, DescTools, 
  ggbeeswarm,
  lme4, nlme, merTools,
  easystats, patchwork
) 
...
```{r}
rawdata <- foreign::read.spss(
  file = here("data/Zellbeads.sav"),
...
ggplot(rawdata, aes(Passage, Growth)) +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 10, 1), limits = c(0, 6)) +
...
#### Model comparison ANOVA-like against regression-like

```{r}
anova(regressionOut, AnovaOut)
```
...
pt_out
pairwise.t.test(
  x = rawdata$Growth, g = rawdata$Passage_F,
  p.adjust.method = "fdr"
)
pairwise.t.test(
  x = rawdata$Growth, g = rawdata$Passage_F,
...
c(
  pt_out$p.value[1, 1], pt_out$p.value[3, 2],
  pt_out$p.value[5, 1]
)
...
(result <- tibble(
  predictor = rownames(anova_out),
  p = formatP(anova_out$"Pr(>F)", ndigits = 5)
))
broom::tidy(anova_out)
```

## compare_n_numvars() as reporting option
...
raw_out$results |>
  select(Variable, `p(ANOVA)` = multivar_p, contains("fn")) |>
