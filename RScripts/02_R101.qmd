...
When there are many tests to do, `case_when`/`case_replace` or `recode_values`/`replace_values` (`case_match` is deprecated) are nice replacements for `ifelse.` While case_when allows complex conditions, recode_values is used for for simple comparisons:
