...
ggpairs(
  rawdata |> select(
    ...
    rename_with(\(x) str_replace(x, "(.+)_(.+)_.+", "\\1 \\2") |>
      str_wrap(8)),
...
var2_rec <-
  recipe(rawdata) |>
  update_role(all_of(predvars2$names), new_role = "predictor") |>
...
# Create the plot
ggplot(v2data, aes(
  .data[[predvars2$names[1]]],
  .data[[predvars2$names[2]]]
)) +
  geom_point(aes(color = species)) +
  geom_abline(
    intercept = 0, slope = slope_pc1,
    color = "black", linetype = "dashed"
  ) +
  geom_segment(
    aes(
      xend = (.data[[predvars2$names[1]]] +
...
pc1plot <-
  ggplot(v2data |> slice(1, 101, 201, 301), aes(
    .data[[predvars2$names[1]]],
    .data[[predvars2$names[2]]]
  )) +
...
pca_preped <-
  pca_rec_trained |>
  step_pca(all_numeric_predictors(), id = "pca_step") |>
  prep()
...
pca_loadings |>
  group_by(component) |>
  slice_max(abs(value), n = 10) |> # Get top 10 influencers per PC
  ungroup() |>
  ggplot(aes(
    x = abs(value), y = reorder_within(terms, abs(value), component),
    fill = value > 0
  )) +
  geom_col() +
  scale_y_reordered() + # Works with reorder_within for clean facets
  facet_wrap(vars(component), scales = "free_y") +
  labs(
    x = "Absolute value of loading",
    y = NULL,
    fill = "Positive?",
...
pca_loadings |>
  ...
  geom_segment(aes(xend = PC1, yend = PC2),
    x = 0, y = 0, arrow = arrow(length = unit(0.2, "cm"))
  ) +
  geom_text(aes(label = terms), check_overlap = TRUE, vjust = 1, hjust = 1) +
  theme_minimal() +
  labs(title = "Variable Loadings Biplot (PC1 vs PC2)")
```
