...
## No age effect, no treatment effect, interaction treatment with agegroup
...
## Age effect, no treatment effect, interaction treatment with agegroup
...
## Age effect, treatment effect, interaction treatment with agegroup
...
ggplot(rawdata, aes(
  ...
  # geom_boxplot()+
  ggbeeswarm::geom_beeswarm(dodge.width = .75, alpha = .25) +
  stat_summary(
    fun = mean,
    aes(group = Treatment)
  ) +
...
lmout_add <- lm(Outcome ~ Agegroup + Treatment,
  data = rawdata |> filter(Agegroup != "old")
)
tidy(lmout_add) |>
  select(1:2)
```

## Interaction in regression-like lm

```{r}
set.seed(1111)
rawdata <- tibble(
  Agegroup = factor(
    rep(c("young", "middle", "old"), each = 1000),
    levels = c("young", "middle", "old")
  ),
  BMI = rnorm(3000,25,1.5)
) |>
  mutate(Outcome = 
           case_when(
             Agegroup == "young" ~
...
lmout <- lm(Outcome ~ Agegroup * BMI,
  data = rawdata
)
tidy(lmout) |>
  mutate(p.value = formatP(p.value, mark = TRUE)) |> 
  select(1:2, 5)
