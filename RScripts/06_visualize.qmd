...
***scale\_***y_continuous(breaks = seq(0,50,2)) + \# tuning of translation of data into aesthetics\
...
[comprehensive overview for color palettes](https://pmassicotte.github.io/paletteer_gallery/){target="_blank"}
...
my_colors <- c("gold", "cornflowerblue", "violet", "#FF012F", "#01AABC")
...
#| eval: false
ggsave(
  filename = here("Graphs/ggtestplot.png"),
  plot = printplot,
  width = 20, height = 20,
  units = "cm", dpi = 150
)
ggsave(
  filename = here("Graphs/ggtestplot.tiff"),
  plot = printplot,
  width = 20, height = 20,
  units = "cm", dpi = 600
)
ggsave(
  filename = here("Graphs/ggtestplot_c.tiff"),
  plot = printplot,
...
# alternative:
png(
  filename = here("Graphs/ggtestplot2.png"),
...
ggplot(mtcars, aes(x = factor(gear), y = wt)) +
  geom_boxplot(coef = 3) +
  geom_dotplot(
    alpha = .7, # group similar(ish) data on a line
...
ggplot(data = mtcars, aes(wt, mpg, color = qsec, size = carb)) +
  scale_color_gradient2(
    low = "red", high = "darkblue",
    mid = "white",
    limits = c(13, 23), midpoint = 18
  ) +
  geom_point() +
  scale_x_continuous(
    breaks = seq(-10^3, 10^3, 5),
...
diamonds |>
  ...
  ggplot(aes(clarity, color = cut, group = cut)) +
  geom_line(stat = "count", position = "identity", lwd = 1) +
...
ggplot(data = mtcars, aes(wt, mpg)) +
  ...
  geom_smooth(
    data = mtcars |> filter(wt > 3), # picks a sub-sample
...
# ? lm for all?
ggplot(data = mtcars, aes(wt, mpg)) +
  geom_smooth(linewidth = 1, color = "black", fill = "yellow") +
  geom_point(size = 3, aes(color = factor(cyl), shape = factor(am))) # aes for geom only
ggplot(data = mtcars, aes(wt, mpg, color = factor(cyl))) +
  geom_smooth(linewidth = 1, color = "black", fill = "yellow") + # global color overwritten
...
There is an informative blog post [Styling plots](https://www.tidyverse.org/blog/2025/10/ggplot2-styling/){target="_blank"} that is a good starter.
