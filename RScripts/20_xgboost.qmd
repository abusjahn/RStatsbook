...
# boostdata <- rawdata |> 
#   select(predvars_n$names) |> 
#   as.matrix()
```

```{r}
xgb_out <- xgboost(
  x = rawdata |> select(predvars$names),#boostdata,
  y = rawdata$species,
  # num_class=3,
  nrounds = 100,
  objective = "multi:softprob")#, # requests class probabilities
  #nthread=1) 
prediction <- predict(xgb_out,rawdata) |> 
  as_tibble() |> 
  mutate(
    ID=rawdata$ID) |> 
